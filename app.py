...
def clean_and_analyze_t2m_data(df):
    ...
    monthly_data = monthly_data[monthly_data['valid_days'] >= 0.5 * monthly_data['total_days']]

    if monthly_data['monthly_avg'].isna().any():
        print("Warning: NaN detected in monthly averages, filling with safe defaults.")
    monthly_data['monthly_avg'] = monthly_data['monthly_avg'].fillna(0)  # Replace NaN with 0

    monthly_data['trend'] = monthly_data['monthly_avg'].diff().fillna(0)  # Replace NaN trends with 0
...
def forecast_with_trends(monthly_data, num_months=4):
    ...
    avg_trends = {month: (trend if not pd.isna(trend) else 0) for month, trend in avg_trends.items()}
...
def calculate_topic_forecasts_with_text(parameter_forecasts):
    return {
        ...
        'Solar Energy Efficiency': calculate_solar_energy_efficiency(parameter_forecasts),
        'Infrastructure Climate Risk Trend': calculate_climate_risk_trend(parameter_forecasts),
    ...
    for i in range(len(data)):
        if data[i] == -float('inf'):
            if i > 0:
                ...
                data[i] = 0
    return data
def llm_generate_insight(analysis_type, forecast_values, business_type, location, search_objective, main_problems):
...
def obter_cidade_estado(latitude, longitude):
    try:
        ...
        cidade = (dados.get('address', {}).get('city') or
                  ...
                  dados.get('address', {}).get('hamlet') or
                  'Desconhecida')

        estado = dados.get('address', {}).get('state', 'Desconhecido')
...
def process_forecasting(input_json):
    ...
    parameter_forecasts = forecast_all_parameters_simple(api_url_base, latitude, longitude)
    ...
    cidade, estado = obter_cidade_estado(latitude, longitude)
...
business_name = st.text_input("Business Name", "Teste LTDA")
business_type = st.selectbox("Tipo de negócio", ["Têxtil", "Mineração"])
st.subheader("Selecione sua localização")
default_location = [-16.6799, -49.255]
...
if "last_clicked" in st_map and st_map["last_clicked"] is not None:
...
analysis_type = st.multiselect("Selecione os tipos de análises", analysis_type_options)

search_objective = st.text_area("Objetivo com a análise", "Reduzir custos operacionais relacionados ao consumo de água e energia.")

main_problems = st.text_area("Principais problemas", "Escassez hídrica em períodos críticos e altos custos de energia elétrica.")

if st.button("Insights gerados"):
    input_json = {
        "business_name": business_name,
        "business_type": business_type,
        "location": {
    ...
    output_json = process_forecasting(input_json)
    st.subheader("Recomendações")

    for insight in output_json["insights"]:
        analysis_type = insight["analysis_type"]
        graph_data = insight["graph_data"]
        insight_tip = insight["insight_tip"]

        st.markdown(f"### {analysis_type}")
        labels = graph_data["labels"]
        data = graph_data["datasets"]["data"]
        label = graph_data["datasets"]["label"]
        ...
        st.text_area("Insight", insight_tip, height=100, key=analysis_type)